 ...
 *  - move.wav есть только у Скаута/Атлета(=tank)/Жнеца — у Солдата (папка
 *    "athlet", RUNNER_TYPES.ATHLETE) своего звука шага нет вообще, для него
 *    используется общий FALLBACK_MOVE_SOUND (тот же lazer.mp3, что раньше
 *    был звуком шага для ВСЕХ типов до этой правки).
 *  - voice-реплики (active/damagedActive) есть только у 3 обычных бегунов
 *    (Скаут/Солдат/Атлет) — у Жнеца и Мяча их нет вообще (ability-based
 *    юниты, не участвуют в обычном SELECT).
 *  - у Скаута damaged-реплики названы БЕЗ "_active_" в файле (scout_damaged_N,
 *    не scout_damaged_active_N, как у Солдата/Атлета) — учтено при require().
 *  - у Атлета(tank) damaged-реплик только 2 (не 3, как у Скаута/Солдата).
 *  - shoot есть у ВСЕХ 4 "стреляющих" типов (Скаут/Солдат/Атлет/Жнец) — Мяч
...
/** Заменяет отсутствующий move-звук у типа (сейчас — только Солдат/ATHLETE, см. доку выше). */
