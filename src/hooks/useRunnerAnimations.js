...
// "смертью" на клетке wall (2026-09-12, см. GameBoardScreen — механика
// Death) — по прямому решению пользователя эта поза САМА ПО СЕБЕ служит
// финальным кадром (отдельная 'destroyed' после неё не нужна), обрабатывается
// в advanceQueue ТЕМ ЖЕ терминальным путём, что и 'destroyed'.
//
...
export function useRunnerAnimations() {
    ...
    const advanceQueue = useCallback((runnerId) => {
        ...
        if (!pending) {
            ...
            // вызывается РОВНО в момент, когда шаг РЕАЛЬНО начинает играть
            // (а не когда trigger() был вызван — тот мог просто встать в
            // очередь позади ещё играющего 'move'/'fly', см. живую жалобу
            // "collision у death начал играть сразу, бегун не успел дойти").
            // Для pending-заготовок onStart НЕ вызывается тут — сама поза
            // ещё не показана (см. комментарий выше), а для kind, которые
            // реально используют onStart (acid/burn), pending никогда не
            // true, так что это не создаёт риска пропустить вызов.
            onStart?.();
        }
        if (toPosition) {
