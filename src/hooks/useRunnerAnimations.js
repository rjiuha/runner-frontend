...
//
// 'bomb' — та же дыра, что была у 'destroyed' (см. DESTROYED_HIDE_DELAY_MS
// выше, найдено и там, и тут 2026-09-26 одной живой жалобой на Жнеца-ловушку):
// `drone_bomb_*.gif` — РОВНО 9 кадров×200мс=1800мс, прежнее значение 1800
// совпадало 1-в-1, без запаса. Тот же запас 400мс — 1800-400=1400.
const ANIM_DURATION_MS = { move: 1440, attack: 1800, gotShot: 1400, fly: 2200, start: 1800, bomb: 1400, heal: 2200 };
...
//
// **Реальный найденный баг (2026-09-26, живая жалоба на вебе — "скаут мигнул,
// доиграл destroy, потом снова мелькнул первый кадр")**: значение 1800 было
// РОВНО полной длиной ОДНОГО проигрыша gif (9 кадров×200мс=1800мс, проверено
// PIL на всех 3 типах×healthy/damaged) — БЕЗ запаса вообще. Тот же класс бага,
// что уже ловили и чинили для acid/burn (2026-09-12) и start (2026-09-13) —
// но именно 'destroyed' тогда пропустили (это отдельный путь, HIDE, а не
// revert-to-idle) и с тех пор никто не возвращался: пока веб был на
// SpritePackAnimation (2026-09-23/24) баг был физически невозможен (там
// настоящий onComplete, не угаданный таймер) — только после возврата веба на
// gif (2026-09-24) этот путь снова стал жить, и дыра всплыла. Тот же запас
// (2 кадра = 400мс), что уже применяли для acid/burn/start — 1800-400=1400.
const DESTROYED_HIDE_DELAY_MS = 1400;
